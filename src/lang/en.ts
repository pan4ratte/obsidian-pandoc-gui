export default {
  ...
  EXT_MADE: 'They come with the plugin, nothing to download',
  ...
  EXT_REFERENCE_DESC: 'reference.docx, reference.odt and reference.pptx — Pandoc’s own documents, carried by the plugin. Open one, set its styles the way you want them and name it under “Reference document” — which is how a Word export starts looking like yours.',
  ...
  EMBED_SHIFT_HEADINGS: 'Fit the headings of embedded notes',
  EMBED_SHIFT_HEADINGS_DESC: 'An embedded note’s headings move one level below the heading it stands under: “# Chapter 1” under “# Part one” is formatted as “## Chapter 1”.',
  DRAWING_FORMAT: 'Excalidraw drawing format',
  DRAWING_FORMAT_DESC: 'In PDF and LaTeX drawings are written as PNG: vector is written as \\includesvg, which needs the svg package and Inkscape. Choose SVG if you have them.',
  DRAWING_FORMAT_PNG: 'PNG',
  DRAWING_FORMAT_SVG: 'SVG',
  FLOAT_PLACEMENT: 'Keep images in place',
  FLOAT_PLACEMENT_DESC: 'In PDF and LaTeX a captioned image is a floating object, and LaTeX places it on its own, leaving a gap in the text. Switch this on to pin images where they stand.',
  ...
  REFERENCE_DOC_GENERATE: (extension: string) => `Generate reference document (${extension})`,
  REFERENCE_DOC_GENERATING: 'Generating…',
  REFERENCE_DOC_MADE: (path: string) => `Reference document created: ${path}`,
  REFERENCE_DOC_OVERWRITE: (path: string) => `${path} is already there. Write it again? Any styles changed in it are lost.`,
  ...
  STORE_EDIT: 'Edit',
  STORE_CANCEL: 'Cancel',
  STORE_SAVED_NOTICE: (name: string) => `Saved “${name}”.`,
  STORE_SAVE_FAILED: (reason: string) => `Save failed: ${reason}`,
  STORE_ADD_OWN: 'Add my own lua-filter',
  STORE_ADD_NAME: 'Filter name',
  STORE_ADD_DESCRIPTION: 'What it does (optional)',
  STORE_ADD_CODE: '-- the lua filter’s code',
  STORE_ADD_PICK_FILE: 'Choose a file',
  STORE_ADD_SAVE: 'Save',
  STORE_ADD_READ_FAILED: (file: string) => `Could not read “${file}”.`,
};
